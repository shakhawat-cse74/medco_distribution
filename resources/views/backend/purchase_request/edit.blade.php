            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex align-items-center">
                        <h4>{{__('Edit Purchase Request')}} ({{ $lims_purchase_request_data->reference_no }})</h4>
                    </div>
                    <div class="card-body">
                        <p class="italic"><small>{{__('db.The field labels marked with are required input fields')}}.</small></p>
                        <form action="{{ route('purchase_requests.update', $lims_purchase_request_data->id) }}" method="POST" enctype="multipart/form-data" id="purchase-request-form">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <div class="col-md-12">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select required name="supplier_id" class="selectpicker form-control" data-live-search="true" id="supplier-id" title="Select Supplier...">
                                                    ...
                                                    <option value="{{$supplier->id}}" @if($supplier->id == $lims_purchase_request_data->supplier_id) selected @endif>{{$supplier->name . ' (' . ($supplier->company_name ?: 'Individual') . ')'}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select id="warehouse_id" name="warehouse_id" required class="selectpicker form-control" data-live-search="true" title="Select warehouse...">
                                                    ...
                                                    <option value="{{$warehouse->id}}" @if($warehouse->id == $lims_purchase_request_data->warehouse_id) selected @endif>{{$warehouse->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="table-responsive mt-3">
                                                <table id="myTable" class="table table-hover order-list">
                                                    ...
                                                    <tbody id="pr-order-tbody">
                                                        @foreach($lims_product_purchase_request_data as $item)
                                                            @php
                                                            ...
                                                            <tr data-id="{{ $item->product_id }}">
                                                                ...
                                                                <td><input type="number" class="form-control qty" name="qty[]" value="{{ $item->qty }}" step="any" min="1" required/></td>
                                                                <td><input type="number" class="form-control net_unit_cost" name="net_unit_cost[]" value="{{ number_format($item->net_unit_cost, 2, '.', '') }}" step="any" min="0" required/></td>
                                                                <td><input type="number" class="form-control discount" name="discount[]" value="{{ number_format($item->discount, 2, '.', '') }}" step="any" min="0"/></td>
                                                                <td class="tax-cell">0.00</td>
                                                                <td><input type="text" class="form-control subtotal" name="subtotal[]" value="{{ number_format($item->total, 2, '.', '') }}" readonly/></td>
                                                    ...
                                                    <tfoot class="tfoot active">
                                                        ...
                                                        <th id="total-qty">{{ number_format($lims_purchase_request_data->total_qty, 2) }}</th>
                                                        <th></th>
                                                        <th id="total-discount">{{ number_format($lims_purchase_request_data->total_discount, 2) }}</th>
                                                        <th id="total-tax">{{ number_format($lims_purchase_request_data->total_tax, 2) }}</th>
                                                        <th id="total">{{ number_format($lims_purchase_request_data->total_cost, 2) }}</th>
                                                        <th><i class="ti ti-trash"></i></th>
                                                    </tfoot>
                                                </table>
                                            </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <input type="hidden" name="total_qty" id="hidden-total-qty" value="{{ $lims_purchase_request_data->total_qty }}" />
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <input type="hidden" name="total_discount" id="hidden-total-discount" value="{{ $lims_purchase_request_data->total_discount }}" />
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <input type="hidden" name="total_tax" id="hidden-total-tax" value="{{ $lims_purchase_request_data->total_tax }}" />
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <input type="hidden" name="total_cost" id="hidden-total-cost" value="{{ $lims_purchase_request_data->total_cost }}" />
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <input type="hidden" name="item" id="hidden-item" value="{{ $lims_purchase_request_data->item }}" />
                                                <input type="hidden" name="order_tax" id="hidden-order-tax" value="{{ $lims_purchase_request_data->order_tax }}" />
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <input type="hidden" name="grand_total" id="hidden-grand-total" value="{{ $lims_purchase_request_data->grand_total }}" />
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row mt-3">
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="order_tax_rate" id="order-tax-rate">
                                                    ...
                                                    <option value="{{$tax->rate}}" @if($lims_purchase_request_data->order_tax_rate == $tax->rate) selected @endif>{{$tax->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <input type="number" name="order_discount" id="order-discount" class="form-control" step="any" value="{{ $lims_purchase_request_data->order_discount ?: 0 }}" />
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label>{{__('db.Shipping Cost')}}</label>
                                                <input type="number" name="shipping_cost" id="shipping-cost" class="form-control" step="any" value="{{ $lims_purchase_request_data->shipping_cost ?: 0 }}" />
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="status">
                                                    <option value="1" @if($lims_purchase_request_data->status == 1) selected @endif>{{__('db.Pending')}}</option>
                                                    <option value="2" @if($lims_purchase_request_data->status == 2) selected @endif>{{__('db.Sent')}}</option>
                                                    <option value="3" @if($lims_purchase_request_data->status == 3) selected @endif>{{__('db.Ordered')}}</option>
                                                    <option value="4" @if($lims_purchase_request_data->status == 4) selected @endif>{{__('db.Draft')}}</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                @if($lims_purchase_request_data->document)
                                                    <div class="mt-1">
                                                        <small>Current document: <a href="{{ asset('public/documents/purchase_request/' . $lims_purchase_request_data->document) }}" target="_blank">{{ $lims_purchase_request_data->document }}</a></small>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <textarea rows="5" class="form-control" name="note">{{ $lims_purchase_request_data->note }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group">
