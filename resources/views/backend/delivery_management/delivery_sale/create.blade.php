...
<section id="pos-layout" class="forms hidden-print">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <h4>{{__('db.Add Delivery Sale')}}</h4>
                <p class="italic"><small>{{__('db.The field labels marked with are required input fields')}}.</small></p>
                <form action="{{ route('delivery-sale.store') }}" method="post" enctype="multipart/form-data" class="payment-form">
                @csrf
                    <div class="card">
                        <div class="card-body">                            
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="delivery_man_id" id="delivery_man_id" class="form-control selectpicker" data-live-search="true" title="Select Delivery Man...">
                                            ...
                                            <option value="{{$dm->id}}" data-routes="{{ implode(',', $dm->assigned_routes ?: []) }}">{{$dm->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
...
$('#warehouse_id').on('change', function() {
        ...
        if (warehouse_id) {
            $.ajax({
                url: '{{ url("/delivery-sale/get-warehouse-products") }}/' + warehouse_id,
                type: 'GET',
                success: function (data) {
                    $results.empty();
                    if (data && data.length > 0) {
                        ...
                        data.forEach(function (product) {
                            ...
                            if (product.is_imei == '1' || product.is_imei === 1 || product.is_imei === true) {
                                productHtml = `<div class="product-img" data-code="${product.code}" data-qty="${product.qty}" data-imei="${product.is_imei}" data-embedded="${product.is_embeded}" data-batch="${batch_id}" data-price="${product.price}">${product.name} (${product.code}) | ${product.price} ${displayStock}</div>`;
                            } else {
                                productHtml = `<div class="product-img" data-code="${product.code}" data-qty="${product.qty}" data-imei="${product.is_imei}" data-embedded="${product.is_embeded}" data-batch="${batch_id}" data-price="${product.price}">${product.name} (${product.code}) | ${product.price} ${displayStock}</div>`;
                            }
    ...
    $('#route_id').on('change', function() {
        ...
        var firstVisibleDM = null;

        if (!routeId) {
            $('#delivery_man_id option').each(function() {
                $(this).show();
                if (firstVisibleDM === null && $(this).val()) {
                    firstVisibleDM = $(this).val();
                }
            });
        } else {
            $('#delivery_man_id option').each(function() {
                var routesAttr = $(this).data('routes');
                var routes = routesAttr ? String(routesAttr).split(',') : [];
                if (routes.length === 0 || routes.indexOf(routeId) !== -1) {
                    $(this).show();
                    if (firstVisibleDM === null && $(this).val()) {
                        firstVisibleDM = $(this).val();
                    }
        ...
        if (firstVisibleDM) {
            $('#delivery_man_id').val(firstVisibleDM).selectpicker('refresh');
        }

        if (routeId) {
            $('#delivery_man_id option:visible').first().prop('selected', true);
