...
<section>
    <div class="container-fluid">
        @can('delivery-installments-add')
            <a href="{{route('delivery-installment.create')}}" class="btn btn-info add-sale-btn btn-icon"><i class="ti ti-plus"></i> {{__('db.Add Installment')}}</a>
        @endcan

        <button type="button" class="btn btn-warning btn-icon" id="toggle-filter">
            <i class="ti ti-filter"></i> {{ __('db.Filter Installments') }}
        </button>
    ...
    <div class="table-responsive">
        <table id="installment-table" class="table installment-list" style="width: 100%">
            <thead>
                <tr>
                    ...
                    <th>{{__('db.Installment Status')}}</th>
...
<script type="text/javascript">
    ...
    $("ul#delivery #delivery-installment-menu").addClass("active");
    ...
    var table = $('#installment-table').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            url: "{{ route('delivery-installment.installmentData') }}",
            data: function(d) {
    ...
    $('#warehouse_id, #delivery_man_id, #route_id').on('change', function() {
        table.ajax.reload();
    });
