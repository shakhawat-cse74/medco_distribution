<style>
/* Sidebar Category Flyout Hover Fix */
.category-list .sidebar > li {
    position: relative;
}
.category-list .sidebar > li.has-dropdown > ul.dropdown {
    position: absolute;
    left: 100%;
    top: 0;
    min-width: 250px;
    ...
    border-left: 2px solid #578B45;
}
.category-list .sidebar > li.has-dropdown:hover > ul.dropdown {
    visibility: visible;
    opacity: 1;
}
.category-list .sidebar > li.has-dropdown > a {
    display: flex;
    justify-content: space-between;
    align-items: center;
}
/* Custom Category Flyout Hover Fix for header */
.category-menu-item .sidebar > li {
    position: relative;
    border-bottom: 1px solid #f1f1f1;
}
.category-menu-item .sidebar > li:last-child {
    border-bottom: none;
}
.category-menu-item .sidebar > li.has-dropdown > a::after {
    content: "›";
    font-size: 18px;
    margin-left: auto;
    font-weight: bold;
}
.category-menu-item .sidebar > li.has-dropdown:hover > ul.dropdown {
    visibility: visible !important;
    opacity: 1 !important;
}
.category-list .sidebar > li.has-dropdown > a::after {
    content: "›";
    font-size: 18px;
    margin-left: auto;
    font-weight: bold;
}
.category-list .sidebar > li.has-dropdown > ul.dropdown > li {
    padding: 0;
    list-style: none;
}
.category-list .sidebar > li.has-dropdown > ul.dropdown > li > a {
    display: block;
    padding: 10px 20px;
    color: #444;
    font-size: 14px;
...
.category-list .sidebar > li.has-dropdown > ul.dropdown > li > a:hover {
    color: #578B45;
    background: #f4faf4;
}
</style>
<header>
        <div class="announcement-bar" style="background-color: #578B45; color: white; padding: 15px 0; text-align: center; position: relative; overflow: hidden;">
            <div class="container-fluid" style="position: relative;">
                <span class="material-symbols-outlined prev-announcement" style="position: absolute; left: 15px; top: 50%; transform: translateY(-50%); cursor: pointer; font-size: 20px; z-index: 10; user-select: none;">chevron_left</span>
                
                <div class="announcement-content" style="position: relative; height: 21px;">
                    <span class="announcement-item" style="font-size: 14px; position: absolute; left: 0; right: 0; transition: opacity 0.3s; opacity: 1;"><strong>Free Shipping</strong> sitewide on orders over <strong>$300</strong></span>
                    <span class="announcement-item" style="font-size: 14px; position: absolute; left: 0; right: 0; transition: opacity 0.3s; opacity: 0; pointer-events: none;"><strong>Orders over $1,800</strong> receive <strong>10% OFF Automatically</strong></span>
                </div>

                <span class="material-symbols-outlined next-announcement" style="position: absolute; right: 15px; top: 50%; transform: translateY(-50%); cursor: pointer; font-size: 20px; z-index: 10; user-select: none;">chevron_right</span>
            </div>
        </div>
                        ...
                        <div class="main-header-inner">
                            <div id="main-menu" class="main-menu">
                                <nav id="mobile-nav" class="d-flex justify-content-between">
                                    <ul>
                                        <li class="user-menu category-menu-item">
                                            <a href="#"><i class="material-symbols-outlined" style="vertical-align: middle; margin-right: 5px;">menu</i> Categories</a>
                                            <ul class="user-dropdown-menu sidebar" style="min-width: 250px; padding: 0;">
                                                @php $i = 0 @endphp
                                                @foreach($all_categories as $category)
                                                    @if(in_array($category->id, $parents))
                                                    @php
                                                    $categories = $categories_list->where('parent_id', $category->id)->where('is_active', 1);
                                                    @endphp
                                                    @if(count($categories) > 0)
                                                    <li class="has-dropdown" style="position: relative;"><a href="{{ url('shop') }}/{{ $category->slug }}" style="padding: 10px 15px; display: flex; align-items: center;">@if(isset($category->icon))<img src="{{ url('images/category/icons/') }}/{{ $category->icon }}" alt="{{ $category->name }}" style="margin-right: 10px;">@endif <span>{{ $category->name }}</span></a>
                                                        <ul class="dropdown" style="position: absolute; left: 100%; top: 0; min-width: 250px; background-color: #fff; box-shadow: 0px 4px 15px rgba(0,0,0,0.1); visibility: hidden; opacity: 0; transition: all 0.2s ease-in-out; z-index: 999; padding: 10px 0; border-left: 2px solid #578B45;">
                                                            @foreach($categories as $cat)
                                                            <li><a href="{{ url('shop') }}/{{ $cat->slug }}" style="padding: 10px 20px; display: block;">@if(isset($cat->icon))<img src="{{ url('images/category/icons/') }}/{{ $cat->icon }}" alt="{{ $cat->name }}" style="margin-right: 10px;">@endif <span>{{ $cat->name }}</span></a></li>
                                                            @endforeach
                                                        </ul>
                                                    </li>
                                                    @else
                                                    <li><a href="{{ url('shop') }}/{{ $category->slug }}" style="padding: 10px 15px; display: flex; align-items: center;">@if(isset($category->icon))<img src="{{ url('images/category/icons/') }}/{{ $category->icon }}" alt="{{ $category->name }}" style="margin-right: 10px;">@endif <span>{{ $category->name }}</span></a>
                                                    </li>
                                                    @endif
                                                    @endif
                                                    @php $i++ @endphp
                                        ...
                                        @if(!empty($topNavItems))
                                            @foreach($topNavItems as $nav)
                                                @if($nav->name == 'Browse By Brands' || $nav->title == 'Browse By Brands' || $nav->name == 'Browse By Brand' || $nav->title == 'Browse By Brand' || $nav->title == 'Brands')
                                                    @continue
                                                @endif
