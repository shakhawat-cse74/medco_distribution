...
class DeliverySaleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (isset($data['pay_term_no']) && is_array($data['pay_term_no'])) {
            $data['pay_term_no'] = $data['pay_term_no'][0] ?? null;
        }
        if (isset($data['pay_term_period']) && is_array($data['pay_term_period'])) {
            $data['pay_term_period'] = $data['pay_term_period'][0] ?? null;
        }
        if (isset($data['paid_amount'])) {
            $data['paid_amount'] = is_array($data['paid_amount']) ? array_sum($data['paid_amount']) : $data['paid_amount'];
        }
        if (isset($data['paid_by_id']) && is_array($data['paid_by_id'])) {
            $data['paid_by_id'] = $data['paid_by_id'][0] ?? null;
        }
        if (isset($data['paying_amount']) && is_array($data['paying_amount'])) {
            $data['paying_amount'] = $data['paying_amount'][0] ?? null;
        }
        if (isset($data['payment_note']) && is_array($data['payment_note'])) {
            $data['payment_note'] = $data['payment_note'][0] ?? null;
        }
        if (isset($data['sale_note']) && is_array($data['sale_note'])) {
            $data['sale_note'] = $data['sale_note'][0] ?? null;
        }
        if (isset($data['staff_note']) && is_array($data['staff_note'])) {
            $data['staff_note'] = $data['staff_note'][0] ?? null;
        }
        ...
        try {
            ...
            // Store payment records if any - simplified for now
            ...
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Delivery sale creation failed: ' . $e->getMessage()
                ], 422);
            }
