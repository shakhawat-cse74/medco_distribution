...
class RunScenario5 extends Command
{
    protected $signature = 'test:scenario5';
    protected $description = 'Run Scenario 5: Create a sale with full due';
    ...
    public function handle()
    {
        ...
        $initialDue = $existing_sales_total - $existing_paid_total;
        ...
        $postData = [
            ...
            'subtotal' => [$total],
            'imei_number' => [null],
            'topping_product' => [null], // JSON string or null
            ...
            'shipping_cost' => 0,
            'grand_total' => $total,
            'payment_status' => 2, // Due
            'paid_amount' => [$paid],
            'paying_amount' => [$paid],
            'amount' => [$paid],
            'account_id' => $account->id,
            'paid_by_id' => [1], // Cash
            'payment_note' => 'Scenario 5 POS Full Due Payment',
            'pos' => 1, // Flag for POS Sale
            'draft' => 0,
            'coupon_active' => 0,
        ];

        // StoreSaleRequest validates input
        ...
        // Verify Customer Due (since 0 paid, due should increase by $total)
        ...
        $actualDue = $new_existing_sales_total - $new_existing_paid_total;
        $expectedDue = $initialDue + ($total - $paid); 
        ...
        $actualRegisterPaymentTotal = Payment::where('cash_register_id', $register->id)->sum('amount');
        // Since $paid is 0, register payment total should stay exactly what it was previously
        $expectedRegisterPaymentTotal = Payment::where('cash_register_id', $register->id)->sum('amount');
        
        $this->info("### Cash Register Verification");
        $this->info("Expected Register Payment Total: $expectedRegisterPaymentTotal");
        $this->info("Actual Register Payment Total: $actualRegisterPaymentTotal");
        $this->info("Result: " . ($expectedRegisterPaymentTotal == $actualRegisterPaymentTotal ? "PASS\n" : "FAIL\n"));
        ...
        $expectedAccountBalance = $account->initial_balance 
            + Payment::where('account_id', $account->id)->sum('amount'); // No increase because $paid = 0
